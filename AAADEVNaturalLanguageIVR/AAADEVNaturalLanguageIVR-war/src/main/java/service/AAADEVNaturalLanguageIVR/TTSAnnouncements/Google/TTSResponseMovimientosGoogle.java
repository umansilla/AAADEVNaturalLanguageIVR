...
/**
 * Clase creada para realizar audio de numero de movimeintos del usuario usando Google Cloud Plataform.
 * @author umansilla
 *
 */
public class TTSResponseMovimientosGoogle {
	private final Call call;
	private final Usuario usuario;
	private static final Logger logger = Logger.getLogger(TTSResponseMovimientosGoogle.class);
	/**
	 * Constructor TTSResponseMovimientosGoogle
	 * @param call Objeto Call del SDK de Avaya Breeze.
	 * @param usuario Objeto Usuario Bean.
	 */
	public TTSResponseMovimientosGoogle(final Call call, final Usuario usuario) {	
		this.call = call;
	 ...
	 */
	public void userIdentifyResponseMovimientos() throws ClientProtocolException, SSLUtilityException, IOException, URISyntaxException, NoAttributeFoundException, ServiceNotFoundException {
		...
		if (AttributeStore.INSTANCE.getServiceProfilesAttributeValue(call.getCalledParty(), Constants.IDIOMA_OPCION).equals("es")) {
			sb.append("Se han encontrado "+usuario.getHistoricoList().size()+" actividades registradas. ");
			for(int i = 0; i < usuario.getHistoricoList().size(); i++){
				sb.append(usuario.getHistoricoList().get(i));
				sb.append(". ");
			}
			
			text = sb.toString() + " Favor de indicar en que más podemos apoyar y al finalizar presione #";
			voice = "es-ES";
		...
		if (AttributeStore.INSTANCE.getServiceProfilesAttributeValue(call.getCalledParty(), Constants.IDIOMA_OPCION).equals("pt")) {
			sb.append("Encontramos  "+usuario.getHistoricoList().size() +" atividades registradas. ");
			for(int i = 0; i < usuario.getHistoricoList().size(); i++){
				sb.append(usuario.getHistoricoList().get(i));
				sb.append(". ");
			}
			
			text = sb.toString() + " Por favor, indique o que mais podemos apoiar e no final pressione #";
			voice = "pt-BR";
		...
		if (AttributeStore.INSTANCE.getServiceProfilesAttributeValue(call.getCalledParty(), Constants.IDIOMA_OPCION).equals("en")) {
			sb.append("We have found  "+usuario.getHistoricoList().size() +" registered activities ");
			for(int i = 0; i < usuario.getHistoricoList().size(); i++){
				sb.append(usuario.getHistoricoList().get(i));
				sb.append(". ");
			}
			
			text = sb.toString() + " Please indicate what else we can support and at the end press #";
			voice = "en-US";
	 ...
	 * @throws URISyntaxException Error se lanza al crear una ruta HTTP incorrecta o inexistentes.
	 */
	private void playTTSResponseMovimientos() throws URISyntaxException{
		/*
		...
		sb.append("http://").append(trafficInterfaceAddress)
				...
				.append("TTSResponseMovimientos.wav");
		...
		JSONObject json = new JSONObject(responseGoogle);
		 if (json.has("audioContent")) {
			 String realPath = getApplcatonPath();
               ...
               final FileOutputStream saveAudioFile = new FileOutputStream(path.toString() + "/TTSResponseMovimientos.wav");
	 ...
	 * @throws IOException Señala que se ha producido una excepción de E / S de algún tipo. Esta clase es la clase general de excepciones producidas por operaciones de E / S fallidas o interrumpidas.
	 */
	private static byte[] decode(byte[] encodedAudioBytes)
